    ...
    if is_linux:
        ...
        config = {
            'UDED': {
                ...
                'data_dir': '/home/xuzhi/lxt/TEED-main/datasets/UDED',  # mean_rgb
                'yita': 0.5,
            ...
            'BIPED': {
                ...
                'data_dir': '/home/xuzhi/lxt/TEED-main/datasets/BIPED/edges',  # mean_rgb
                'yita': 0.5,
                'mean':BIPED_mean
            #
            },
        ...
        config = {
            'UDED': {
                ...
                'train_list': None,
                'test_list': 'test_pair.lst',
                'data_dir': r'F:\python-project\TEED-main\datasets\UDED',  # mean_rgb
                'yita': 0.5,
            ...
            'BIPED': {'img_height': 720,  # 720
                      ...
                      'train_list': 'train_rgb.lst',
                      # 'data_dir': 'F:/datasets/BIPED/BIPED/BIPED/edges',  # WIN: '../.../dataset/BIPED/edges'
                      'data_dir': 'F:/python-project/MBIPED-master/MBIPED-master/BIPED/edges',
                      'yita': 0.5,
                      'mean':BIPED_mean},
            'BIPED-B2': {'img_height': 720,  # 720
                      'img_width': 1280,  # 1280
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image = cv2.bilateralFilter(src=image, d=21, sigmaColor=30, sigmaSpace=30)#修改,双边滤波预处理
        #image = cv2.equalizeHist(image)#应用直方图均衡化,不支持多线程
        #image = cv2.convertScaleAbs(image,alpha=3.0,beta=0)#修改对比度，alpha表示对比度修改，beta表示亮度偏移
...
class BipedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 应用双边滤波
        # image = cv2.bilateralFilter(src=image, d=21, sigmaColor=30, sigmaSpace=30)
        ...
        #    transforms.ToTensor()
        #])
        #tensor_image = transform(image).unsqueeze(0)  # 在批量维度上增加一个维度

        # 对图像进行高斯模糊处理
        #blurred_tensor = gaussian_blur(tensor_image, kernel_size=[5, 5], sigma=(0.7,0.7))

        # 将PyTorch张量转回为OpenCV图像
        #blurred_image = transforms.ToPILImage()(blurred_tensor.squeeze(0))
        #image = cv2.cvtColor(np.array(blurred_image), cv2.COLOR_RGB2BGR)
        ##image = cv2.bilateralFilter(src=image, d=21, sigmaColor=30, sigmaSpace=30)#修改,双边滤波预处理
        ##image = cv2.equalizeHist(image)#应用直方图均衡化,不支持多线程
        ##image = cv2.convertScaleAbs(image,alpha=3.0,beta=0)#修改对比度，alpha表示对比度修改，beta表示亮度偏移
